 ...
 * 
 * Updated by Manolis Mariakakis to provide concurrently multiple instances
 * 
 */
...
(function () {
    ...
    window.lc_color_picker = function (attachTo, options = {}) {
        ...
        // If the attachTo argument is a string, assume it's an element ID
        if (typeof attachTo === 'string') {
            // Ensure it is an ID selector and select the element by ID
            attachTo = document.getElementById(attachTo);
        }

        if (!attachTo) {
            return console.error('You must provide a valid input element or selector as the first argument');
        }
        ...
        this.show_picker = function (trigger) {
            ...
            if (typeof window.selectedObject === 'undefined' || !window.selectedObject) {
                const errorMessage = document.getElementById("error-message");
                errorMessage.style.display = "flex"; 
                errorMessage.textContent = "Please select an object!"; 
                setTimeout(() => {
            ...
            if (document.querySelector('#lc-color-picker.lccp-shown[data-trigger-id="' + active_trig_id + '"]')) {
                ...
                active_trigger = null;
                active_trig_id = null;
                window.openedPickerId = null; // Reset when closed
                return false;
            }
            ...
            // Set openedPickerId to the currently active trigger ID
            window.openedPickerId = attachTo.id;

            this.val_to_picker();
            this.append_color_picker();

            const picker = document.getElementById('lc-color-picker'),
                parentDiv = trigger.parentNode, // Target the parent div
                parentRect = parentDiv.getBoundingClientRect(); // Get the parent div's position

            // Position picker directly below the parent div
            const topPosition = parentRect.bottom + window.scrollY; // Get bottom of parent div
            const leftPosition = parentRect.left + window.scrollX;  // Align with the left of the parent div

            picker.style.position = 'absolute';
            picker.style.top = `${topPosition}px`;
            picker.style.left = `${leftPosition}px`;
            picker.style.maxWidth = '200px'; // Set max-width to 200px
            picker.style.width = 'auto'; // Allow the picker to take its natural width
        ...
        this.apply_gradient_changes = function (also_apply_changes) {
            ...
            new_gradient += (active_mode == 'linear-gradient') ? gradient_data.deg + 'deg' : (gradient_data.radial_circle) ? 'circle' : 'circle';
            new_gradient += ', ';
        ...
        this.mode_change = function (el, new_mode) {
            ...
            if (options.modes.length > 1) {
                ...
                document.querySelector('.pccp_circle_f_wrap').style.display = (new_mode == 'radial-gradient') ? 'none' : 'none';
            }
